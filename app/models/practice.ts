...
class PracticeModel {
  ...
  static async findByDialstackAccountId(accountId: string): Promise<Practice | null> {
    const pool = await dbConnect();

    try {
      const result = await pool.query('SELECT * FROM practices WHERE dialstack_account_id = $1', [
        accountId,
      ]);

      return result.rows[0] || null;
    } catch (error) {
      throw new Error(`Failed to find practice by DialStack account ID: ${error}`);
    }
  }
  ...
  static async emailExists(email: string): Promise<boolean> {
    ...
    try {
      const result = await pool.query('SELECT COUNT(*) FROM practices WHERE email = $1', [email]);
  ...
  static async updateEmail(currentEmail: string, newEmail: string): Promise<Practice> {
    const pool = await dbConnect();
  ...
  static async updateConfig(email: string, config: PracticeConfig): Promise<Practice> {
    const pool = await dbConnect();
