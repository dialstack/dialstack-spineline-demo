...
export async function POST(request: NextRequest) {
  ...
  try {
    event = DialStack.webhooks.constructEvent<CreateBookingWebhook>(
      body,
      signature,
      webhookSecret,
    );
  ...
  const endAt = new Date(
    startAt.getTime() + event.booking.duration_minutes * 60 * 1000,
  );

  // Check for conflicting appointments
  const conflicts = await AppointmentModel.findConflicting(
    practice.id,
    startAt,
    endAt,
  );
  ...
  if (event.booking.customer.phone) {
    const patient = await PatientModel.findByPhone(
      practice.id,
      event.booking.customer.phone,
    );
    if (patient) {
