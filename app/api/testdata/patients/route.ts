...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!count || count < 1 || count > 20) {
      return new Response(JSON.stringify({ error: 'Invalid count. Must be between 1 and 20.' }), {
        status: 400,
    ...
    for (let i = 0; i < count; i++) {
      ...
      try {
        ...
        logger.warn({ error, patientData }, 'Failed to create patient, possibly duplicate email');
