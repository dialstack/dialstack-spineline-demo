...
export function PatientQuickInfo({
  ...
  const { lastActivity, nextActivity, outstandingBalance } =
    generatePatientActivity(patient.id!);
  ...
  return (
    <div className="rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
      ...
      <div
        className={`${compact ? "p-3 space-y-2 text-xs" : "p-4 space-y-2.5 text-sm"}`}
      >
        ...
        <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
          ...
          <span className="font-medium ml-auto text-right">
            {nextActivity
              ? formatFutureDate(nextActivity.date)
              : "None scheduled"}
          </span>
        </div>
