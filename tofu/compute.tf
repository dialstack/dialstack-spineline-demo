...
resource "aws_instance" "blue" {
  ...
  root_block_device {
    ...
    tags = {
      Name = "${var.project_name}-blue-root"
    }
  }
...
# Green Environment EC2 Instance
resource "aws_instance" "green" {
  ...
  root_block_device {
    ...
    tags = {
      Name = "${var.project_name}-green-root"
    }
  }
...
# Elastic IPs for application servers (for stable DNS pointing)
resource "aws_eip" "blue" {
  count = var.deploy_blue ? 1 : 0

  instance = aws_instance.blue[0].id
  domain   = "vpc"

  tags = {
    Name = "${var.project_name}-blue-eip"
  }

  depends_on = [aws_internet_gateway.main]
}

resource "aws_eip" "green" {
  count = var.deploy_green ? 1 : 0

  instance = aws_instance.green[0].id
  domain   = "vpc"

  tags = {
    Name = "${var.project_name}-green-eip"
  }
